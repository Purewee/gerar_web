...
import { useCategories, useFeaturedCategories, type Category } from '@/lib/api';
import { useCategoriesStore } from '@/lib/stores/categories';

const CATEGORIES_CACHE_KEY = 'categories_cache';
const FEATURED_CATEGORIES_CACHE_KEY = 'featured_categories_cache';
...
export function CategoriesProvider({ children }: { children: React.ReactNode }) {
  ...
  const [cachedFeaturedCategories] = useState<Category[] | null>(() => {
    if (typeof window === 'undefined') return null;
    try {
      const cached = sessionStorage.getItem(FEATURED_CATEGORIES_CACHE_KEY);
      if (!cached) return null;
      const parsed = JSON.parse(cached);
      return Array.isArray(parsed) ? parsed : null;
    } catch {
  ...
  const hydrateFeaturedFromCache = useCategoriesStore(state => state.hydrateFeaturedFromCache);
  const setFeaturedCategories = useCategoriesStore(state => state.setFeaturedCategories);
  const setLoading = useCategoriesStore(state => state.setLoading);
  const hydratedFromCacheRef = useRef(false);
  const hydratedFeaturedFromCacheRef = useRef(false);
  ...
  if (
    cachedFeaturedCategories &&
    cachedFeaturedCategories.length > 0 &&
    !hydratedFeaturedFromCacheRef.current
  ) {
    hydratedFeaturedFromCacheRef.current = true;
    hydrateFeaturedFromCache(cachedFeaturedCategories);
  }

  const { data: categoriesResponse, isLoading } = useCategories({
    enabled: !cachedCategories,
  });
  const { data: featuredCategoriesResponse } = useFeaturedCategories({
    enabled: !cachedFeaturedCategories,
  });
  ...
  }, [featuredCategoriesResponse, setFeaturedCategories]);
