...
import LoyaltyProductClient from './gift-product-client';

async function getGiftPointProduct(id: string) {
...
export async function generateMetadata({
  ...
  const product = await getGiftPointProduct(id);
...
export default async function LoyaltyProductPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
